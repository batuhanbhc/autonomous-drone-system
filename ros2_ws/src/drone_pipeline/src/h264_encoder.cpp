...
void H264Encoder::open(int width, int height, int fps, int gop_size)
{
  ...
  av_opt_set(codec_ctx_->priv_data, "crf",    "28",           0);
...
H264Encoder::EncodeResult H264Encoder::encode(const std::vector<uint8_t> & jpeg_data)
{
  ...
  // Re-initialise swscale only when the decoded pixel format changes
  // (should be constant in practice, but cheap to guard against).
  const int src_fmt = raw_frame_->format;
  if (!sws_ctx_ || src_fmt != last_src_fmt_) {
    if (sws_ctx_) { sws_freeContext(sws_ctx_); sws_ctx_ = nullptr; }
    sws_ctx_ = sws_getContext(
      width_, height_, static_cast<AVPixelFormat>(src_fmt),
  ...
  sws_scale(sws_ctx_,
            raw_frame_->data, raw_frame_->linesize, 0, height_,
