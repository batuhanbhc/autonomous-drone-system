...
def infer_actor_state_grid_channels(actor_state: dict) -> int:
    legacy_key = "cnn.net.0.conv.weight"
    if legacy_key in actor_state:
        return int(actor_state[legacy_key].shape[1])
    people_key = "cnn.people_branch.net.0.conv.weight"
    context_key = "cnn.context_branch.net.0.conv.weight"
    if people_key in actor_state and context_key in actor_state:
        return int(actor_state[people_key].shape[1] + actor_state[context_key].shape[1])
...
def infer_checkpoint_include_shared_count_memory_staleness_channel(ckpt: dict) -> bool:
    return bool(ckpt.get("include_shared_count_memory_staleness_channel", False))
...
def infer_base_actor_grid_channels(
    ...
    include_shared_count_memory_staleness_channel: bool,
    include_instant_fov_channels: bool = True,
) -> int:
    return (
        6
        ...
        + int(bool(include_shared_count_memory_staleness_channel))
    )
...
def infer_shared_people_channels(
    ...
    if shared_people_channels not in {1, 2, 3, 4, 5, 6}:
        raise ValueError(
            "Unsupported actor grid layout: expected "
            f"{fixed_non_shared_channels + 1}-{fixed_non_shared_channels + 6} "
...
def add_shared_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=SHARED_DEFAULTS.include_shared_count_memory_staleness_channel,
        help="Expose a shared staleness/confidence channel for historic count memory in actor and critic inputs.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=SHARED_DEFAULTS.count_memory_decay_grace_period_seconds,
        help="Grace period before passive count-memory decay starts for cells that are no longer visible.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=SHARED_DEFAULTS.count_map_compression_scale,
        help="Compression scale c used for count maps via x / (x + c). Larger values compress more aggressively.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Framing-quality field used by the FOV quality reward. Top-corner mode uses principal-point distance normalized by the farther top-corner radius.",
    )
    parser.add_argument(
        ...
        help=(
            "Exponent for gamma-based principal reward quality modes. gamma=1 "
            "matches their linear variant; higher values penalize off-center "
            "framing sooner."
        ),
    )
    parser.add_argument(
...
def build_env_kwargs(
    ...
    include_shared_count_memory_staleness_channel = getattr(
        args,
        "include_shared_count_memory_staleness_channel",
        SHARED_DEFAULTS.include_shared_count_memory_staleness_channel,
    )
    ...
    if actor_grid_channels is None:
        actor_grid_channels = infer_base_actor_grid_channels(
            include_local_recent_count_memory_channel=(
                include_local_recent_count_memory_channel
            ),
...
def actor_kwargs(
    ...
    return {
        ...
        "include_instant_fov_channels": bool(include_instant_fov_channels),
        "include_persistent_coverage_channel": bool(
            include_persistent_coverage_channel
        ),
    }
